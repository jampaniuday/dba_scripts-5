 ...
 * the Oracle JDBC OCI driver to perform the following actions:
 * <ul>
...
public class JdbcExampleOCI  {

    static final String programName = "JdbcExamples.JdbcExampleOCI";
     ...
     *             program.
     */
    public JdbcExampleOCI(String[] args) {
        ...
        for (int i=0; i < args.length; i++) {
            ...
            if (args[i].equalsIgnoreCase("-tns")) {
                tns = args[++i];
                foundTns = true;
            }
        ...
        // --------------------------------------------------------
        if (!foundTns || !foundUser || !foundPassword) {
            printUsage();
        }

        driver_class  = "oracle.jdbc.driver.OracleDriver";
        connectionURL = "jdbc:oracle:oci8:@" + tns;
        userID        = user;
    ...
    public static void main(String[] args) {
        ...
        JdbcExampleOCI jdbcExampleOCI = null;
        
        try {
            
            jdbcExampleOCI = new JdbcExampleOCI(args);
            jdbcExampleOCI.getConnection();
            jdbcExampleOCI.createTable();
            jdbcExampleOCI.insertValues();
            jdbcExampleOCI.queryForward();
            jdbcExampleOCI.queryReverse();
            jdbcExampleOCI.deleteRows();
            jdbcExampleOCI.dropTable();
            jdbcExampleOCI.genericQuery();
            
        } catch (SQLException e) {
            System.out.println("ERROR: SQLException");
            if (jdbcExampleOCI.conn != null) {
                try {
                    jdbcExampleOCI.conn.rollback();
