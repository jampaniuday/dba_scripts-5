...
/
/*select distinct a.osuser, a.terminal, a.machine, A.sid,B.type,A.username,' alter system kill session   '''||A.sid||','||A.serial#||''';  ' "SID_Serial",A.status,B.block,A.lockwait,
        substr(to_char(A.LOGON_TIME, 'dd/mon hh:mi'), 1, 12) "Logon Time"
 FROM V$SESSION A,V$LOCK B
 WHERE A.SID=B.SID
 AND USERNAME IS NOT NULL
 AND lockwait is not null
order by substr(to_char(A.LOGON_TIME, 'dd/mon hh:mi'), 1, 12)
*/
